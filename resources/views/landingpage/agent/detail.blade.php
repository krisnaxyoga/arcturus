                                                                                ...
                                                                                foreach ($HotelCalendar as $key => $calendar) {
                                                                                    ...
                                                                                    if ($calendar->room_hotel_id == $itemprice->room_id && $calendar->active == 1) {
                                                                                        ...
                                                                                        if ($startDate < $checkoutDate && $endDate >= $checkinDate && $foundZero == false && $startDate != $endDate) {
                                                                                            ...
                                                                                            {
                                                                                                $ratecalender = $calendar->recom_price;
                                                                            ...
                                                                            <div class="col-lg">
                                                                                ...
                                                                                @endphp
                                                                                    ...
                                                                                    {{-- @foreach ($blackoutdate as $blackoutdateitem)
                                                                                        @if($blackoutdateitem->vendor_id == $item->contractrate->vendors->id && $blackoutdateitem->contract_id == $itemprice->contractrate->id)
    ...
    <script>
        ...
        function calculateTotal() {
            ...
            for (var i = 0; i < roomQuantities.length; i++) {
                ...
                var agentmarkup = parseInt(roomQuantities[i].options[roomQuantities[i].selectedIndex].dataset.agentmarkup);
                var noagentmarkup = parseInt(roomQuantities[i].options[roomQuantities[i].selectedIndex].dataset.noagentmarkup);
        ...
        $('.category').change(function() {
            ...
            if (nilaiInput == 0) {
                ...
                $.ajax({
                    url: "{{ route('agent.hotelmarkup.hoteldetail', $data[0]->contract_id) }}",
                    method: 'GET',
                    data: {
                        data: {
                            category: nilaiInput
        ...
        $('#booking').off('click').on('click', function() {
            ...
            $.ajax({
                url: "{{ route('agent.hotelmarkup.create') }}",
                method: 'POST',
                data: {
                    _token: "{{ csrf_token() }}",
                ...
                success: function(response) {
                    ...
                    var id = response[0];
                    localStorage.clear();
                    window.location.href = "{{ route('agent.hotelmarkup.detail', ['id' => ':id']) }}"
                        .replace(':id', id);
                    // window.location.reload();
                },
                error: function(xhr) {
