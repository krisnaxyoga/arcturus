...
export default function Index({
    ...
    const handleFileChange = (e) => {
        ...
        if (file) {
            ...
            if (!allowedExtensions.includes(fileExtension)) {
                alert(
                    "Only image files with formats png, jpg, or jpeg are allowed!"
                );
                e.target.value = ""; // Mengosongkan input file
            } else if (file.size > maxFileSizeInBytes) {
                alert(
                    "File size must not exceed 5 MB!, otherwise contact admin"
                );
                e.target.value = ""; // Mengosongkan input file
            } else {
                setLogo(file);
    ...
    const handleCopyCode = () => {
        ...
        try {
            ...
            const message = successful
                ? "Copied successfully!"
                : "Failed to copy. Use Ctrl+C (Cmd+C on Mac) to copy.";
            //   console.log(message);
            alert(message);
        } catch (err) {
    ...
    const generateRandomCode = () => {
        const confirmSave = window.confirm(
            "Out from current management, please click the button below to create a New Management Code, then press the save button to save the changes. if you are sure? press yes"
        );
        if (confirmSave) {
        ...
        if (isSelected) {
            setSelectedDistribute((prevSelectedValues) =>
                prevSelectedValues.filter(
                    (value) => value !== selectedDistributes
                )
            );
        } else {
            setSelectedDistribute((prevSelectedValues) => [
    ...
    const storePost = async (e) => {
        ...
        if (confirmSave) {
            ...
            formData.append(
                "country",
                selectcountry ? selectcountry : data[0].country
            );
            formData.append(
                "busisnessname",
                busisnessname ? busisnessname : data[0].vendor_name
            );
            formData.append("email", email ? email : data[0].users.email);
            formData.append(
                "firstname",
                firstname ? firstname : data[0].users.first_name
            );
            formData.append(
                "lastname",
                lastname ? lastname : data[0].users.last_name
            );
            formData.append("phone", phone ? phone : data[0].phone);
            formData.append("logo", logo ? logo : data[0].logo_img);
            formData.append(
                "address",
                address ? address : data[0].address_line1
            );
            formData.append(
                "address2",
                address2 ? address2 : data[0].address_line2
            );
            ...
            formData.append(
                "highlight",
                highlight ? highlight : data[0].highlight
            );
            formData.append(
                "type_property",
                type_property ? type_property : data[0].type_property
            );

            formData.append(
                "code",
                managementcode ? managementcode : data[0].users.title
            );
            // formData.append('tax', taxValue ? taxValue : markup.tax);
            formData.append("bank", bankname ? bankname : data[0].bank_name);
            formData.append(
                "bankaccount",
                bankaccount ? bankaccount : data[0].bank_account
            );
            formData.append(
                "swifcode",
                swifcode ? swifcode : data[0].swif_code
            );
            formData.append(
                "bankaddress",
                bankaddress ? bankaddress : data[0].bank_address
            );
            formData.append(
                "accountnumber",
                accountnumber ? accountnumber : data[0].account_number
            );
            formData.append(
                "email_reservation",
                emailreservation ? emailreservation : data[0].email_reservation
            );
                                                        ...
                                                        <div>
                                                            ...
                                                            <div className="input-group mb-3">
                                                                ...
                                                                {data[0].country == 'Indonesia' ? (
                                                                    ...
                                                                    {Object.keys(indonesiaprovinsi).map((key) => (
                                                                        <option
                                                                            key={key}
                                                                            value={key}
                                                                            selected={
                                                                ...
                                                                <div className="form-group">
                                                                    ...
                                                                    <span className="d-flex">
                                                                        ...
                                                                        <button
                                                                            ...
                                                                            type="button"
                                                                            title="generate new management code"
                                                                        >
                                                                            {" "}
                                                                        ...
                                                                        (
                                                                            ...
                                                                            >
                                                                                <span
                                                                                    ...
                                                                                    className="btn badge badge-success text-light mx-1"
                                                                                >
                                                                                    {
                                                                                        value
                                                                                    }{" "}
                                                                            ...
                                                                            >
                                                                                ...
                                                                                <td>
                                                                                    <Link
                                                                                        ...
                                                                                        href={`/myprofile/slider/delete/${item.id}`}
                                                                                    >
